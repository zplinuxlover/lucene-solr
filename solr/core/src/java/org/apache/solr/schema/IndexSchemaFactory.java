  ...
  public IndexSchema create(String resourceName, SolrConfig config) {
    ...
    if (config.getConfigSetService() instanceof CloudConfigSetService) {
      ...
      classLoader = new PackageAwareSolrClassLoader(
          ...
          () -> configSetService.refreshAllSchema(config.getConfigsetName()),
    ...
    IndexSchema schema = new IndexSchema(resourceName, inputSource, config.luceneMatchVersion, classLoader, config.getConfigsetName());
    return schema;
  }
